...
def correlation_test():
    ...
    C4 = reg.correlation(I*0.75,J*0.75)
    assert C4 < 1, '??'
...
def ngradient_test():
    ...
    g2 = reg.ngradient(lambda x: x**2, np.array([2.0]))
    print(g2)
    assert abs(g2- 4) < 1e-5, 'oops'
    
    g3 = reg.ngradient(lambda x: x**3, np.array([2.0]))
    print(g3)
    assert abs(g3- 12) < 1e-5, 'oops'
...
def registration_metrics_demo(use_t2=False):
    ...
    angles = np.linspace(-np.pi, np.pi, 10, endpoint=True)
